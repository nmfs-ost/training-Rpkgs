...
Follow instructions in [R packages book system setup](https://r-pkgs.org/setup.html) to work locally. Note you can use your preferred IDE, if it is not Rstudio.
...
```
Running the code above will create the data file which contains your data as a `.rda` file. The data can be accessed with `data("your_data")`. You don't have to keep the data-raw folder after converting your data to `.rda`, although it is recommended to [preserve a dataset's origin story](https://r-pkgs.org/data.html#sec-data-data-raw).
