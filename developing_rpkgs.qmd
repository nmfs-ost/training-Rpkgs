...
9am - 12:30 pm (followed by lunch 12:30 - 2:30 pm)
...
The source code for the cowsay R package [is available on GitHub](https://github.com/sckott/cowsay). We'll take a look at its anatomy to better understand R packages but at it's base, an R package is essentially a collection of functions.
...
- What do you want to name your package? (packages have specific naming conventions, no "_" in names are allowed)
...
- Which R packages would you need to rely on to run the code in your package (in addition to cowsay)?
...
# Will need to provide a file path with path = "your_filepath" if not working within a project at first. After you run create_package(), a R project will be created for your package.
```
...
```
Remember that you NEVER want to use `library()` before a function but rather `usethis::use_package("package")` to include a required package in the imports and you will always want to use the `package::function` convention, e.g. `cowsay::say()`.

The `invisible()` function is useful instead of `return()` when you don't want the returned output to be printed, such as all of the output of a model.
...
A shortcut to add the {roxygen2} skeleton documentation is to click inside the function and then for Window: Ctrl+Alt+Shift+R and for Mac: Cmd+Option+Shift+R which will give you this:
```{r}
#' Title
#'
#' @param words 
...
You can also use `usethis::use_package_doc()` followed by to create a package documentation file that will show up when you use `?packagename`.
...
If you would like to add a vignette to your package you can use `usethis::use_vignette("vignette_name")` which will set up a skeleton vignette as a .Rmd file.

### Adding data to your package
If there is data that you would like to make available in the package, you can include it using `usethis::use_data_raw()` which will create a folder called "data_raw" where you will paste/copy in your dataset. From there, you will edit the DATASET.R file that was automatically created.
```{r}
...
Debugging is necessary if your tests are failing. `traceback()` is one way that R code can be debugged. The Rstudio IDE is best integrated with this approach, although it can be used with other IDEs as well. Another useful function for debugging a function is the `browser()` function.
