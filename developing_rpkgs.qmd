...
::: {.callout-tip}
## Which version converntions should I use?
...
A released version number consists of three numbers, `<major>.<minor>.<patch>`. More details can be found in the [Lifecycle Chapter of R packages](https://r-pkgs.org/lifecycle.html#sec-lifecycle-version-number). 
:::
...
A shortcut to add the {roxygen2} skeleton documentation is to click inside the function and then for Windows: `Ctrl+Alt+Shift+R` and for Mac: `Cmd+Option+Shift+R` which will give you this:
```{r}
#| eval=FALSE
#' Title
#'
...
file. NEVER edit the `.Rd` and `NAMESPACE` files directly. `NAMESPACE` file is generated with the {roxygen2} package, using specific tags (e.g., `@export`) located in a rosygen comment above each functions's definition. A specific namespace disambiguates functions with the same name (e.g., `remotes::install_github()` and `devtools::install_github()`). 
...
### Style R code
`styler::style_pkg()` style R code according to the [tidyverse style guide](https://style.tidyverse.org/). It helps to keep the coding style consistent across files.
